# With Claude , it gets the objects from verified_words, rating them for specific variables listed in this algorithm, puts them in data_a.json file which is one of the required datas to run the algorithm
...
def get_rating(object_name, category):
    while True:
        try:
            rawrating = (get_claude_response(f"Give me an approximate rating for the object {object_name}'s {category} from (0 to 1000): "))
